...
mod tests {
    ...
    impl Retriever for FakeRetriever {
        async fn retrieve(&self, _query: &str, top_k: usize) -> AgentResult<Vec<ScoredDocument>> {
            let mut guard = self.last_top_k.lock().expect("top_k mutex poisoned");
            *guard = Some(top_k);
    ...
    async fn pipeline_passes_top_k() {
        ...
        let seen = *top_k_ref.lock().expect("top_k mutex poisoned");
    ...
    async fn pipeline_propagates_errors() {
        ...
        match err {
            AgentError::ExecutionFailed(message) => assert!(message.contains("retrieval failed")),
