...
mod tests {
    ...
    fn test_register_with_source() {
        ...
        assert!(matches!(source, Some(ToolSource::Builtin)));
    }
    ...
    fn test_tool_searcher() {
        ...
        // 按名称搜索
        let results = searcher.search_by_name("echo");
        assert_eq!(results.len(), 1);

        // 按描述搜索
        let results = searcher.search_by_description("output");
        assert_eq!(results.len(), 1);
    }
}
