...
async fn test_debug_session_api_integration() {
    ...
    for event in &events {
        recorder.record_event(session_id, event).await.expect("failed");
    }

    // End the session
    recorder.end_session(session_id, "completed").await.expect("failed");
    ...
    let retrieved_events = recorder.get_events(session_id).await.expect("failed");
    ...
    let retrieved_session = recorder.get_session(session_id).await.expect("failed");
...
async fn test_debug_session_not_found() {
    ...
    let result = recorder.get_session("non-existent").await.expect("failed");
    assert!(result.is_none(), "Non-existent session should return None");

    // Try to get events for non-existent session
    let events = recorder.get_events("non-existent").await.expect("failed");
    assert!(
        events.is_empty(),
