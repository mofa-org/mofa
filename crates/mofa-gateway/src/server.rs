...
impl GatewayServer {
    ...
    pub fn build_router(&self) -> Router {
        ...
        if let Some(ref _orch_config) = self.orchestrator_config {
            // Use a simple provider name - routing policy can be configured separately
            let bridge = Arc::new(InferenceBridge::new("local"));
            router = router
                .merge(openai_router())
