...
mod tests {
    ...
    #[tokio::test]
    async fn test_in_memory_store_messages() {
        let store = InMemoryStore::new();
        ...
        // 统计
        let count = store.count_session_messages(session_id).await.unwrap();
        assert_eq!(count, 2);
    }

    #[tokio::test]
    async fn test_in_memory_store_api_calls() {
        let store = InMemoryStore::new();
    ...
    #[tokio::test]
    async fn test_in_memory_store_sessions() {
        let store = InMemoryStore::new();

        let user_id = Uuid::now_v7();
        let agent_id = Uuid::now_v7();

        let session = ChatSession::new(user_id, agent_id).with_title("Test Session");
        ...
        // 获取用户会话列表
        let sessions = store.get_user_sessions(user_id).await.unwrap();
        assert_eq!(sessions.len(), 1);
    }

    #[tokio::test]
    async fn test_bounded_store() {
        let store = BoundedInMemoryStore::new(2, 2);
        ...
        assert_eq!(store.inner.message_count().await, 2);
    }
}
