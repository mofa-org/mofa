...
mod tests {
    ...
    #[tokio::test]
    async fn test_parse_and_build_yaml() {
        let yaml = r#"
metadata:
  id: test_workflow
  name: Test Workflow
...
"#;
        ...
        let def = WorkflowDslParser::from_yaml(yaml).unwrap();
        let graph = WorkflowDslParser::build_with_agents(def, &HashMap::new())
            .await
            .unwrap();

        assert_eq!(graph.id(), "test_workflow");
    }

    #[test]
    fn test_env_substitution() {
        std::env::set_var("WORKFLOW_ID", "my_workflow");
        std::env::set_var("WORKFLOW_NAME", "My Workflow");
...
"#;
        ...
        let def = WorkflowDslParser::from_yaml(yaml).unwrap();
        assert_eq!(def.metadata.id, "my_workflow");
        assert_eq!(def.metadata.name, "My Workflow");
    }
...
"#;
        ...
        let def = WorkflowDslParser::from_toml(toml).unwrap();
        assert_eq!(def.metadata.id, "test_workflow");
        assert_eq!(def.nodes.len(), 2);
    }

    #[test]
    fn test_validation_missing_start() {
        let yaml = r#"
metadata:
  id: test_workflow
  name: Test Workflow
...
"#;
        ...
        let def = WorkflowDslParser::from_yaml(yaml).unwrap();
        let result = WorkflowDslParser::validate(&def);
