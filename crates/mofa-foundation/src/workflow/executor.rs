    ...
    async fn execute_from_node(
        ...
        loop {
            ...
            while !ready_queue.is_empty() && running_tasks.len() < self.config.max_parallelism {
                ...
                // Check for HITL review node (unified system or legacy Wait)
                if node.config.node_type == NodeType::Wait {
                    ...
                    if let Some(ref review_handler) = self.review_handler {
                        info!("Requesting review at node: {} (unified HITL)", current_node_id);

                        let review_context = self.create_review_context(ctx, &current_node_id, &current_input).await;

                        match review_handler.request_node_review(&record.execution_id, &current_node_id, review_context).await {
                            Ok(review_id) => {
                                ...
                                *ctx.last_waiting_node.write().await = Some(current_node_id.clone());
                                ...
                                ctx.set_variable("review_id", WorkflowValue::String(review_id.as_str().to_string())).await;

                                running_tasks.abort_all();
                                return Ok(WorkflowValue::Null);
                            }
                            Err(e) => {
                                error!("Failed to request review: {} - falling back to legacy Wait", e);
                            }
                        }
                    }

                    // Legacy Wait node handling (fallback)
                    info!("Pausing workflow at node: {} (legacy Wait)", current_node_id);
                    *ctx.paused_at.write().await = Some(chrono::Utc::now());
                    *ctx.last_waiting_node.write().await = Some(current_node_id.clone());
                ...
                running_tasks.spawn(async move {
                    ...
                    let execution_result = match node_clone.node_type() {
                        ...
                        _ => {
                            ...
                            let node_timeout = std::time::Duration::from_millis(executor_clone.config.node_timeout_ms);
                            match tokio::time::timeout(node_timeout, node_clone.execute(&ctx_clone, current_input.clone())).await {
                                Ok(r) => r,
                                Err(_) => {
                                    warn!("Node {} timed out after {:?}", current_node_id, node_timeout);
                                    NodeResult::failed(
                                        &current_node_id,
