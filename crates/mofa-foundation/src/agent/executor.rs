...
mod tests {
    ...
    async fn test_executor_creation() {
        ...
        let executor = AgentExecutor::new(llm, temp_dir.path()).await.unwrap();

        assert_eq!(executor.config().max_iterations, 10);
        assert_eq!(executor.name(), "LLMExecutor");
        // ID is now a UUID generated by BaseAgent
        assert!(!executor.id().is_empty());
        assert!(executor.id().contains('-')); // UUID format
    }
    ...
    async fn test_process_message() {
        let temp_dir = TempDir::new().unwrap();
        let llm = Arc::new(MockLLMProvider);
        let mut executor = AgentExecutor::new(llm, temp_dir.path()).await.unwrap();

        let response = executor
            .process_message("test:session", "Hello")
            .await
            .unwrap();
    ...
    async fn test_composition() {
        ...
        // Verify BaseAgent composition
        assert_eq!(executor.base().id(), executor.id());
        assert_eq!(executor.base().name(), executor.name());
        assert!(executor.base().capabilities().tags.contains(&"llm".to_string()));
    }
}
