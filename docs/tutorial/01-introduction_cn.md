...
这意味着你可以在不触及核心的情况下替换 LLM 提供者、存储后端、工具注册表甚至脚本引擎。以下是 MoFA 10 个 crate 的分层结构：

```
┌─────────────────────────────────────────────────────┐
│              mofa-sdk（标准 API）                     │
│  你的主要入口点。从下层重新导出你需要的一切。              │
└─────────────────────┬───────────────────────────────┘
                      │
┌─────────────────────▼───────────────────────────────┐
│           mofa-runtime（执行引擎）                    │
│  AgentRunner、AgentRegistry、事件循环、                │
│  生命周期管理                                        │
└─────────────────────┬───────────────────────────────┘
                      │
┌─────────────────────▼───────────────────────────────┐
│         mofa-foundation（具体实现）                   │
│  LLM 提供者（OpenAI、Anthropic、Gemini、Ollama）      │
│  LLMAgent、AgentTeam、工具、持久化、                   │
│  工作流、秘书智能体                                    │
└─────────────────────┬───────────────────────────────┘
                      │
┌─────────────────────▼───────────────────────────────┐
│           mofa-kernel（Trait 定义）                   │
│  MoFAAgent、Tool、Memory、Reasoner、Coordinator、    │
│  AgentPlugin、StateGraph — 仅包含接口                 │
└─────────────────────────────────────────────────────┘
                      │
┌─────────────────────▼───────────────────────────────┐
│          mofa-plugins（插件层）                       │
...
其他 crate：
  mofa-cli        CLI 工具（含 TUI）
  mofa-ffi        UniFFI + PyO3 绑定
...
- **10 个 crate** 形成分层系统：SDK → Runtime → Foundation → Kernel → Plugins
